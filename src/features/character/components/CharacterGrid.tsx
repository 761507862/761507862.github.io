...
export const CharacterGrid: React.FC<CharacterGridProps> = ({ onRecordDungeon }) => {
  ...
  const handleAddCharacter = () => {
    if (newCharName.trim()) {
      addCharacter({ name: newCharName, class: newCharClass, gearScore: newCharGearScore });
      setNewCharName('');
  ...
  return (
    <>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-[300px] flex items-center justify-center rounded-lg border-2 border-dashed border-muted-foreground/25 hover:border-primary/50 transition-colors">
          <Button variant="ghost" className="h-full w-full flex flex-col gap-2" onClick={() => setIsAddModalOpen(true)}>
            <Plus className="h-8 w-8 text-muted-foreground" />
            <span className="text-muted-foreground">{t('character.add')}</span>
          </Button>
        </motion.div>
      </div>
      ...
      <Dialog
        ...
        title={t('character.addModal.title')}
        footer={
          <>
